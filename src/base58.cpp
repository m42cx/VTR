...
namespace {
    class CVentorAddressVisitor : public boost::static_visitor<bool> {
    private:
        CVentorAddress *addr;
    public:
        CVentorAddressVisitor(CVentorAddress *addrIn) : addr(addrIn) { }
...
bool CVentorAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CVentorAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CVentorAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CVentorAddressVisitor(this), dest);
}

bool CVentorAddress::IsValid() const {
...
CTxDestination CVentorAddress::Get() const {
    if (!IsValid())
...
bool CVentorAddress::GetKeyID(CKeyID &keyID) const {
...
bool CVentorAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CVentorSecret::SetKey(const CKey& vchSecret) {
...
CKey CVentorSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CVentorSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CVentorSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CVentorSecret::SetString(const std::string& strSecret) {
