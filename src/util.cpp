...
int nAnonymizeVentorAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Ventor
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Ventor
    // Mac: ~/Library/Application Support/Ventor
    // Unix: ~/.Ventor
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ventor";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Ventor.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Ventord.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Ventor will not work properly.");
