...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeVentorAmount"))
        settings.setValue("nAnonymizeVentorAmount", 1000);
    nAnonymizeVentorAmount = settings.value("nAnonymizeVentorAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeVentorAmount"))
        SoftSetArg("-anonymizeVentoramount", settings.value("nAnonymizeVentorAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeVentorAmount:
            return QVariant(nAnonymizeVentorAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeVentorAmount:
            nAnonymizeVentorAmount = value.toInt();
            settings.setValue("nAnonymizeVentorAmount", nAnonymizeVentorAmount);
            emit AnonymizeVentorAmountChanged(nAnonymizeVentorAmount);
            break;
        default:
            break;
        }
