...
{
    // Convert Ventor:// to Ventor:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Ventor://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Ventor:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Ventor.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Ventor.lnk
...
{
    return GetAutostartDir() / "Ventor.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Ventor.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Ventor\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Ventor app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Ventor app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Ventor-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Ventor-qt [" + tr("command-line options") + "]                     " + "\n";
